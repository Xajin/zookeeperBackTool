...
class TiZkBackup(TiZkBackupRestoreBase):
    def __init__(self, filename, **kwargs):
        ...
        self.db_conn.execute('CREATE TABLE zk_data (key_name CHAR(50) NOT NULL, key_value  VARCHAR(500), PRIMARY KEY (key_name));')
        self.db_conn.commit()
    ...
    def save_node(self, path, is_parent, xml_node):
        ...
        try:
            ...
            print('data: {}'.format(data.decode('gbk', 'ignore')))
            # sqlite数据
            key_name = str(uuid.uuid1())
            self.db_conn.execute('insert into zk_data VALUES (?,?)', (key_name, data.decode('gbk', 'ignore')))  # sqlite3.Binary(data)
            self.db_conn.commit()
...
if __name__ == '__main__':
    ...
    # 测试模式
    if is_test:
        zk_serv_src = '127.0.0.1:2181'
        zk_serv_dst = '127.0.0.1:2181'
